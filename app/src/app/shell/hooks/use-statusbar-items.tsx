...
import { supportsAppAutoUpdate, supportsLocalPty } from '@/lib/web-platform'
...
export function useStatusbarItems({
  ...
  const clientVersionItem = useMemo<StatusbarItem>(() => {
    ...
    return {
      ...
      // The client pill's only action is opening the Electron app-update
      // overlay; in the web build auto-update is unsupported, so hide this
      // dead entry (the backend version pill, which updates over REST, stays).
      hidden: (!appVersion && !sha) || !supportsAppAutoUpdate(),
  ...
  const coreRightStatusbarItems = useMemo<readonly StatusbarItem[]>(
    () => [
      ...
      },
      {
        ...
        // No local PTY in the web build (the terminal bridge is omitted), so
        // hide the toggle rather than surface a button that opens a dead pane.
        hidden: !chatOpen || !supportsLocalPty(),
